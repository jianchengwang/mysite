  ...
  <main class="article-main p-4 max-w-5xl m-auto">
    <header v-if="data.title" class="article-header p-4 pb-12">
      ...
      <ul class="article-tags flex gap-2 py-2" v-if="data.tags && data.tags.length">
        <li class="tag bg-slate-100 text-slate-700 text-sm p-2 py-1 rounded-md transition-all !py-0.5" v-for="(tag, n) in data.tags" :key="n">{{ tag }}</li>
      </ul>
    </header>
    <hr class="article-hr" />
    <section class="article-section grid grid-cols-8">
      ...
      <div class="article col-span-full md:col-span-6 md:col-start-1 md:row-start-1 w-full p-4 max-w-3xl m-auto prose dark:prose-invert" v-html="data.content">
      </div>
    </section>
